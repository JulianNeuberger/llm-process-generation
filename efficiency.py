...
def parse_results(directory_path: str, parse_tags: bool):
    ...
    for power_file in os.listdir(power_directory):
        ...
        counts = get_token_count_from_json(directory_path)
        token_count = counts[current_iter - 1]
        throughput = token_count / time_elapsed
        pow_data.append(
            {
                "kWh": kwh,
...
def boxplot_from_data(directory_path: str, ans_df, pow_df, parse_err, retries):
    ...
    fig, axes = plt.subplots(1, 5, figsize=(18, 6))  # 1 row, 5 columns
    ...
    # Plot token_count
    sns.boxplot(data=pow_df['output tokens'], width=0.5, showmeans=True, ax=axes[3])
    axes[3].set_ylabel('Avg. output token count')

    # Plot throughput
    sns.boxplot(data=pow_df['throughput'], width=0.5, showmeans=True, ax=axes[4])
    axes[4].set_ylabel('Avg. throughput in tok/s')
...
def get_token_count_from_json(directory_path: str):
    counts = []
    answer_directory = os.path.join(directory_path, "answers")
    ...
    for answer_file in os.listdir(answer_directory):
        ...
        with open(filepath, 'r', encoding='utf-8') as file:
            try:
                ...
                # Recursively search for "output_tokens" in all nested structures
                output_tokens += find_output_tokens(file_data)
            except json.JSONDecodeError as e:
                print(f"Error decoding {answer_file}: {e}")

        counts.append(output_tokens)

    return counts


def find_output_tokens(json_data):
    output_tokens = 0

    # If the data is a dictionary, check if it contains "output_tokens"
    if isinstance(json_data, dict):
        ...
        for value in json_data.values():
            output_tokens += find_output_tokens(value)

    # If the data is a list, check each item for "output_tokens"
    elif isinstance(json_data, list):
        for item in json_data:
            output_tokens += find_output_tokens(item)
