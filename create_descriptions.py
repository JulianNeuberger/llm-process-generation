...
if __name__ == "__main__":
    ...
    def create_bp_from_llm(and_prompt_b: bool, examples_prompt_b: bool, count: int, chat_model: BaseChatModel, storage: str):

        text_list = []
        pydantic_parser = PydanticOutputParser(pydantic_object=BP_Description)
        format_instructions = pydantic_parser.get_format_instructions()
        prompt_template =PromptTemplate(
            input_variables=["and_prompt", "examples_prompt"],
        ...
        for x in range(0, count):
            print("Getting "+ x + " Description from "+ count)
